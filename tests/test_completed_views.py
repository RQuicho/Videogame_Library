...
class CompletedViewTestCase(TestCase):
    """Test views for completed library"""
...
#############################################################################################################################
    ...
    def test_show_completed(self):
        with self.client as c:
            ...
            resp = c.get(f"/users/{self.u1id}/completed")
            html = resp.get_data(as_text=True)       

            self.assertEqual(resp.status_code, 200)
            self.assertIn('<a class="nav-link active" aria-current="page" href="/users/111/completed">Completed</a>', html)
    ...
    def test_assign_game_to_completed(self):
        cat1 = Category(
            completed = self.g1.game_id,
            user_id = self.u1id
        )
        cat1id = 1
        cat1.id = cat1id
        ...
        self.assertEqual(cat1.completed, 3328)
