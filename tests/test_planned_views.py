...
class PlannedViewTestCase(TestCase):
    """Test views for planned library"""
...
#############################################################################################################################
    ...
    def test_show_planned(self):
        with self.client as c:
            ...
            resp = c.get(f"/users/{self.u1id}/planned")
            html = resp.get_data(as_text=True)       

            self.assertEqual(resp.status_code, 200)
            self.assertIn('<a class="nav-link active" aria-current="page" href="/users/111/planned">Plan to Play</a>', html)
    ...
    def test_assign_game_to_planned(self):
        cat1 = Category(
            planned = self.g1.game_id,
            user_id = self.u1id
        )
        cat1id = 1
        cat1.id = cat1id
        ...
        self.assertEqual(cat1.planned, 3328)
