...
class AllGamesViewTestCase(TestCase):
    """Test views for all_games library"""
...
#############################################################################################################################
    ...
    def test_show_all_games(self):
        with self.client as c:
            ...
            resp = c.get(f"/users/{self.u1id}/all_games")
            html = resp.get_data(as_text=True)       

            self.assertEqual(resp.status_code, 200)
            self.assertIn('<a class="nav-link active" aria-current="page" href="/users/111/all_games">All Games</a>', html)
