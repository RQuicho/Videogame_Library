...
class PlayedViewTestCase(TestCase):
    """Test views for played library"""
...
#############################################################################################################################
    ...
    def test_show_played(self):
        with self.client as c:
            ...
            resp = c.get(f"/users/{self.u1id}/palyed")
            html = resp.get_data(as_text=True)       

            self.assertEqual(resp.status_code, 200)
            self.assertIn('<a class="nav-link active" aria-current="page" href="/users/111/played">Played</a>', html)
    ...
    def test_assign_game_to_played(self):
        cat1 = Category(
            played = self.g1.game_id,
            user_id = self.u1id
        )
        cat1id = 1
        cat1.id = cat1id
        ...
        self.assertEqual(cat1.played, 3328)
